...
let validateDtoName dto = dto.Name |> validateName |> function | Ok _ -> ok dto | Bad es -> Bad es

let validateDtoEmail dto = dto.EMail |> validateEmail |> function | Ok _ -> ok dto | Bad es -> Bad es
